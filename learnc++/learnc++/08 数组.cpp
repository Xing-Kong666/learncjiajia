...
int main08() {
	...
	*/
	...
	/*
	1、数据类型 数组名【行数】【列数】
	2、数据类型 数组名【2】【3】={{1,2,3},{2,3,5}}
	3、数据类型 数组名【】【3】={1,3,4,3,5,5,4,3,2,5};//不全的后面自动补零
	*/
	int arr02[2][3] = {3,3,5,2,3};
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 3; j++) {
			cout << arr02[i][j] << " ";
		}
	...
	2、查看数组内存地址
	*/
	cout << "二维数组arr02的所占空间为：" << sizeof(arr02) << " byte" << endl;
	cout << "arr02的内存首地址为：" << arr02 << endl;
	cout << "内存首地址10进制表示为：" << (int)arr02 << endl;
	cout << "第一行首地址为：" << arr02[0] << endl;
	cout << "第二行首地址为：" << arr02[1] << endl;
	/*二维数组案例-考试成绩统计*/
	//10个学生，语数外三科
	//提示用户输入班级学生的成绩
	cout << "请输入您班级的10个学生的语数外的成绩：" << endl;
	//用户输入
	int score[10][3];
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 3; j++) {
